...
const templateLoaders: Record<TemplateCategory, () => Promise<TemplateFile>> = {
  presentation: () => import('./categories/presentation.json').then((m) => m.default as TemplateFile),
...
export async function loadTemplatesByCategory(category: TemplateCategory): Promise<PromptTemplate[]> {
...
export async function loadAllTemplates(): Promise<PromptTemplate[]> {
  const categories: TemplateCategory[] = ['presentation', 'education', 'business', 'creative'];
  const results = await Promise.all(categories.map(loadTemplatesByCategory));
  return results.flat();
}

export function getTemplateById(templates: PromptTemplate[], id: string): PromptTemplate | undefined {
